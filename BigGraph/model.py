...
class Encoder(nn.Module):
    def __init__(self,
        ...
        # input projection (no residual)
            ...
            self.norms.append(nn.BatchNorm1d(num_hidden * heads[0],momentum=0.01))
            # hidden layers
            for l in range(1, num_layers-1):
                ...
                self.gat_layers.append(layer(
                    ...
                    feat_drop, attn_drop, negative_slope, False, None))
                self.norms.append(nn.BatchNorm1d(num_hidden * heads[l-1],momentum=0.01))
            # output projection
            self.gat_layers.append(layer(
...
class GraphTL(nn.Module):
    ...
    def get_loss(self, z1, z2, pos_, feat_):
        ...
        top, index = th.topk(pos_, 10, dim=1, sorted=False)
