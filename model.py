...
class GraphTL(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim, num_layers,numheads,K):
        super(GraphTL, self).__init__()
        heads = ([numheads] * (num_layers - 1)) + [1]
        self.encoder = Encoder(num_layers=num_layers,in_dim=in_dim, num_hidden=hid_dim, num_classes=out_dim, heads=heads,
                           activation=F.relu, feat_drop=0.0, attn_drop=0.0, negative_slope=0.2, residual=False)
    ...
    def get_loss(self, z1, z2, pos_, feat_):
        ...
        pos_=pos_
        feat_ = feat_
        pos_ = th.matmul(pos_, pos_.T) #Topological similarity matrix
        feat_ = th.matmul(feat_, feat_.T) #Node attribute similarity matrix
        pos_ = (F.sigmoid(self.alpha) * pos_ + (1-F.sigmoid(self.alpha)) * feat_) #Node similarity matrix
        k = self.K
        top, index = th.topk(pos_, k, dim=1, sorted=False)
